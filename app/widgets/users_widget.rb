  ...
  data do
    {
      ...
      previous: User.distinct.where('users.created_at < ?', Date.today).joins(:history_entries).where(history_entries: { created_at: (4.weeks.ago.beginning_of_week..Time.now) }).count
    }
  end
end
