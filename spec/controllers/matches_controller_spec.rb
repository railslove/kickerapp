...
describe MatchesController, type: :controller do
  describe '#create' do
    ...
    specify do
      post :create, params: { league_id: league.slug, team1: { player1: user.id.to_s, player2: '' }, team2: { player1: user2.id.to_s, player2: '' }, set1: %w[6 3], set2: ['', ''], set3: ['', ''] }
    ...
    it 'redirects to new_league_match if mobile' do
      ...
      post :create, params: { league_id: league.slug, team1: { player1: user.id.to_s, player2: '' }, team2: { player1: user2.id.to_s, player2: '' }, set1: %w[6 3], set2: %w[7 5], set3: ['', ''] }
    ...
    it 'adds a crawling match param if the was a crawling match' do
      post :create, params: { league_id: league.slug, team1: { player1: user.id.to_s, player2: '' }, team2: { player1: user2.id.to_s, player2: '' }, set1: %w[6 0], set2: ['', ''], set3: ['', ''], crawling1: true }
  ...
  describe '#update' do
    ...
    it 'changes nothing if only result changes' do
      ...
      patch :update, params: { id: match.id, league_id: league.id, winner_score: '6', loser_score: '2', crawling: false }
      expect(user.reload.quota).to eql(1208)
    ...
    it 'changes points if crawling changes' do
      allow(subject).to receive(:current_league).and_return(league)
      patch :update, params: { id: match.id, league_id: league.id, winner_score: '6', loser_score: '2', crawling: true }
    ...
    it 'swap teams if winner scored less scores then loser' do
      allow(subject).to receive(:current_league).and_return(league)
      patch :update, params: { id: match.id, league_id: league.id, winner_score: '3', loser_score: '6', crawling: false }
