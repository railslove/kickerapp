...
describe User do
  ...
  describe ".win_percentage" do
    it "takes the QuotaCalculator" do
      user = FactoryGirl.build(:user, number_of_wins: 3, number_of_looses: 7)
      QuotaCalculator.stub(:win_loose_quota).and_return(20)
  ...
  describe ".set_elo_quota" do
    context "win" do
      ...
      it "calculates without crawling" do
        match = double(win_for?: true, winner_team: double("w_team", elo_quota: 1200), looser_team: double("l_team", elo_quota: 1200), crawling: false, difference: 5)
      ...
      it "calculates with crawling (+5)" do
        match = double(win_for?: true, winner_team: double("w_team", elo_quota: 1200), looser_team: double("l_team", elo_quota: 1200), crawling: true, difference: 10)
        subject.set_elo_quota(match)
    ...
    context "loose" do
      ...
      it "calculates without crawling" do
        match = double(win_for?: false, winner_team: double("w_team", elo_quota: 1200), looser_team: double("l_team", elo_quota: 1200), crawling: false, difference: 5)
        subject.set_elo_quota(match)
